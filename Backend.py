...
async def get_adzuna_jobs(q: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        return [
            {
                "id": f"adzuna_{j['id']}",
                "title": j["title"] + " (Adzuna)",
                "company": j["company"]["display_name"],
                "location": f"{j['location']['area'][-1]}, {j['location']['area'][-3]}" if len(j['location'].get('area', [])) >= 3 else j['location']["display_name"],
                "salary": ("Estimated Salary: " f"${int(j.get('salary_min')):,}") if j.get("salary_min") is not None else "N/A",
                "url": j["redirect_url"],
